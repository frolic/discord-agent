...
function renderServiceUnit(input: {
  ...
  const template = readFileSync(resolve(templateDir, "discord-agent.service"), "utf-8");
  return template
    .replaceAll("{{BOT_NAME}}", input.botName)
    .replaceAll("{{USER}}", input.user)
    .replaceAll("{{AGENT_HOME}}", input.agentHome)
    .replaceAll("{{SOURCE_DIR}}", input.sourceDir)
    .replaceAll("{{BUN_PATH}}", input.bunPath);
}
