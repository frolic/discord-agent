...
import { postThreadSeed } from "./thread.ts";
...
describe("postThreadSeed", () => {
  test("empty seed posts nothing", async () => {
    const { post, posted } = makeFakePost();
    await postThreadSeed(post, "");
    expect(posted).toEqual([]);
  });

  test("short seed posts in a single message, unchanged", async () => {
    const { post, posted } = makeFakePost();
    const seed = "Just a short seed message.";
    await postThreadSeed(post, seed);
    expect(posted).toEqual([seed]);
  });

  test("long seed splits into multiple posts, each under hardLimit", async () => {
    const { post, posted } = makeFakePost();
    // ~150 tokens of filler under a 1990-char hard limit — enough to
    // force a split across paragraphs.
    const filler = "word ".repeat(600);
    const seed = `intro paragraph\n\n${filler}\n\nclosing paragraph`;
    await postThreadSeed(post, seed);
  ...
  test("long seed containing a table splits with the header repeated per piece", async () => {
    const { post, posted } = makeFakePost();
