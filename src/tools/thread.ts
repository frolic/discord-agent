...
export function createThreadTool(args: {
  ...
  return defineTool({
    ...
    label: "create thread",
    description:
      "Create a Discord thread in the current channel for multi-step or long-running work. Posts your initial_message as the first message in the thread — that message is visible to the user AND serves as the seed context for the fresh agent session that runs there. Make the initial_message the build brief itself: open with the concrete goal and deliverable, then the user's requirements, constraints, and key context. Do NOT write a generic execution-approach template or lead with process boilerplate — the new session gets everything from this message, and it is the brief the user sees in-channel. If you are ALREADY inside a thread (executing work in a thread session), do not call this tool to re-create or duplicate it — the thread already exists and you are its session; proceed with the work in this thread. Returns the thread ID; the next user message in the thread spins up a brand-new conversation scope, so make initial_message self-contained.",
